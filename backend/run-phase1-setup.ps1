...
psql -U $POSTGRES_USER -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASSWORD';" 2>$null
if ($LASTEXITCODE -eq 0) {
...
psql -U $POSTGRES_USER -d $DB_NAME -c "GRANT USAGE ON SCHEMA public TO $DB_USER;"
psql -U $POSTGRES_USER -d $DB_NAME -c "GRANT CREATE ON SCHEMA public TO $DB_USER;"
...
psql -U $POSTGRES_USER -d $DB_NAME -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL PRIVILEGES ON TABLES TO $DB_USER;"
psql -U $POSTGRES_USER -d $DB_NAME -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL PRIVILEGES ON SEQUENCES TO $DB_USER;"
...
$dbCheck = psql -U $POSTGRES_USER -t -c "SELECT datname FROM pg_database WHERE datname = '$DB_NAME';"
if ($dbCheck -match $DB_NAME) {
    Write-Host "✅ Database exists: $DB_NAME" -ForegroundColor Green
}

$userCheck = psql -U $POSTGRES_USER -t -c "SELECT usename FROM pg_user WHERE usename = '$DB_USER';"
if ($userCheck -match $DB_USER) {
    Write-Host "✅ User exists: $DB_USER" -ForegroundColor Green
}

$postgisCheck = psql -U $POSTGRES_USER -d $DB_NAME -t -c "SELECT extname FROM pg_extension WHERE extname = 'postgis';"
...
Write-Host "⚠️  Server will start in background. Check for errors above." -ForegroundColor Yellow
Write-Host ""

# Start server in background
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd d:\Guardian-Route\backend; npm run start:dev"

Write-Host "⏳ Waiting 10 seconds for server to start..." -ForegroundColor White
